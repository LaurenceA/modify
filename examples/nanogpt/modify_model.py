...
class SDPA(nn.Module):
    ...
    def forward(self, kqv):
        k, q, v = kqv
        B, T, n_embd = k.size() # batch size, sequence length, embedding dimensionality (n_embd)
...
def CausalSelfAttention(config):
    return modify.Sequential([
        nn.Linear(config.n_embd, 3*config.n_embd, bias=config.bias), #B, T, 3*n_embd
        modify.ViewUnMerge(dim=-1, n0=3, n1=config.n_embd),     #B, T, 3, n_embd
